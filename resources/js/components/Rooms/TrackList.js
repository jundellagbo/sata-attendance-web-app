    ...
    const rooms = dataSource.length ? dataSource.map( (row, key ) => {
        return (
            <TableRow hover key={row.id}>
                <TableCell align={"left"} component="th" scope="row">{row.tracks}</TableCell>
                <TableCell align={"right"}>
                    <Buttonicon tooltip={"Go to this Track"} color={"primary"} onClick={() => onRedirect(row.id) }>
                        <Textsms />
                    </Buttonicon>
                    { isAdmin && (
    ...
    return (
        <Paper>
            <Table>
                <TableHead>
                    <TableRow>
                        <TableCell style={style.theadcol} align={"left"}>Track name</TableCell>
